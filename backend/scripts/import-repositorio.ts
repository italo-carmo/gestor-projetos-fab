...
const rbacPath = path.join(rootDir, 'RBAC_MATRIX.json');
...
function readDocxLines(filePath: string) {
...
function readPptxLines(filePath: string) {
...
function readXlsxLines(filePath: string) {
  const list = execFileSync('unzip', ['-Z1', filePath], {
    encoding: 'utf-8',
  })
    .split('\n')
    .filter(Boolean);
  ...
  if (list.includes('xl/sharedStrings.xml')) {
    const sharedXml = execFileSync(
      'unzip',
      ['-p', filePath, 'xl/sharedStrings.xml'],
      {
        encoding: 'utf-8',
        maxBuffer: 50 * 1024 * 1024,
      },
  ...
  const sheetEntries = list.filter((entry) =>
    /^xl\/worksheets\/sheet\d+\.xml$/.test(entry),
  );
  const lines: string[] = [];

  for (const sheetEntry of sheetEntries) {
    const xml = execFileSync('unzip', ['-p', filePath, sheetEntry], {
      encoding: 'utf-8',
      maxBuffer: 50 * 1024 * 1024,
    });
...
function toDateISO(year: number, ddmm: string) {
...
async function clearStorage() {
  const folders = [
    ...
    path.join(process.cwd(), 'storage', 'activity-reports'),
...
function extractDistributionLocalities() {
  const distributionDoc = path.join(
    repoRoot,
    '7. CIPAVD-SMIF/Plano de Ação_CMTAER/PLANO DE AÇÃO JUNTO AO SMIF_CMTAER.docx',
  );
  ...
  for (let i = 0; i < lines.length - 1; i += 1) {
    ...
    if (/^TOTAL DE VAGAS$/i.test(normalize(line))) break;

    const looksLikeLocality = /[A-Za-zÀ-ÿ\s]+-[A-Z]{2}/.test(line);
    const looksLikeNumber = /^\d+$/.test(next);
    if (!looksLikeLocality || !looksLikeNumber) continue;
...
function extractScheduleVisits() {
  const scheduleDocs = [
    path.join(
      repoRoot,
      '7. CIPAVD-SMIF/Proposta de Trabalho/CRONOGRAMA DE VISITAS DA CIPAVD AO SMIF.docx',
    ),
    path.join(
      repoRoot,
      '7. CIPAVD-SMIF/Cronograma de Viagens e Equipe/CRONOGRAMA DE VISITAS DA CIPAVD AO SMIF_versão Equipe.docx',
    ),
  ];
  ...
  for (const scheduleDoc of scheduleDocs) {
    if (!fs.existsSync(scheduleDoc)) continue;
    const lines = readDocxLines(scheduleDoc);
    const joined = lines.join(' | ');

    const matches = [
      ...joined.matchAll(
        /(\d{2}[./]\d{2})(?:\s*a\s*(\d{2}[./]\d{2}))?\s*\|\s*([A-Za-zÀ-ÿ\s]+-\s*[A-Z]{2})/g,
      ),
      ...joined.matchAll(
        /(\d{2}[./]\d{2})(?:\s*a\s*(\d{2}[./]\d{2}))?\s*\|\s*([A-Za-zÀ-ÿ\s]+-\s*[A-Z]{2})\s*-/g,
      ),
    ];
    ...
    for (const match of matches) {
      ...
      if (!/[A-Za-zÀ-ÿ\s]+-\s*[A-Z]{2}/.test(localityName)) continue;
  ...
  for (const item of items) {
    uniq.set(
      `${item.localityName}:${item.startDate.toISOString().slice(0, 10)}`,
      item,
    );
  }
...
function extractAdditionalLocalities() {
  const source = path.join(
    repoRoot,
    '1. CIPAVD - Documentos Gerais /Atividades CIPAVD 2026.docx',
  );
  const lines = readDocxLines(source);
  const set = new Map<string, LocalitySeed>();
...
async function upsertCatalog() {
  ...
  const phases: Array<{ name: PhaseName; order: number; displayName: string }> =
    [
      { name: PhaseName.PREPARACAO, order: 1, displayName: 'Preparação' },
      { name: PhaseName.EXECUCAO, order: 2, displayName: 'Execução' },
      {
        name: PhaseName.ACOMPANHAMENTO,
        order: 3,
...
async function upsertLocalities() {
  ...
  const all = new Map<string, LocalitySeed>();
  ...
  // Garante códigos de GSD presentes no repositório
  const forced = [
  ...
  for (const item of forced) {
    if (!all.has(item.code)) all.set(item.code, item);
  }
...
async function upsertUsers(
  localityByCode: Map<string, { id: string; code: string; name: string }>,
) {
...
async function createMeetings() {
  const meeting = await prisma.meeting.create({
    data: {
      ...
      meetingType: 'ONLINE',
      meetingLink:
        'https://fab.webex.com/fab/ldr.php?RCID=42c8592a11f27ff9e40c674a82f9ca96',
      agenda:
        'Alinhamento do plano de apoio da CIPAVD no SMIF, definição de elos locais e cronograma de visitas.',
    },
  });
...
async function createActivities(
  ...
  const created = new Map<string, { id: string; localityId: string | null }>();

  for (const visit of schedule) {
    const code = makeCodeFromName(visit.localityName);
    const locality = localityByCode.get(code) ?? null;
    const status =
      visit.endDate.getTime() < Date.now() ? 'DONE' : 'NOT_STARTED';

    const activity = await prisma.activity.create({
      data: {
        title: `Palestra CIPAVD - ${visit.localityName}`,
        description:
          'Atividade oficial do cronograma CIPAVD/SMIF: palestra de prevenção e enfrentamento ao assédio moral e sexual, com equipe presencial e coleta de dados.',
  ...
  if (manaus) {
    const mission = await prisma.activity.create({
      data: {
        ...
        description:
          'Missão realizada em 24 e 25 de novembro de 2025, com debriefing, palestras e consolidação de resultados.',
...
async function createTaskBacklog(
  localityByCode: Map<string, { id: string; code: string; name: string }>,
  gsdUsers: Map<string, { id: string; name: string; email: string }>,
) {
  const phases = await prisma.phase.findMany();
  const phaseByName = new Map(phases.map((p) => [p.name, p]));

  const templatesSeed: Array<{
    title: string;
    phase: PhaseName;
    ...
    {
      title:
        'Indicar elos locais (doutrina, psicologia, serviço social e jurídico)',
      phase: PhaseName.PREPARACAO,
      reportRequiredDefault: false,
    ...
    },
    {
      ...
      phase: PhaseName.ACOMPANHAMENTO,
      reportRequiredDefault: true,
      priority: TaskPriority.HIGH,
      offsetDays: 5,
    },
  ...
  for (const seed of templatesSeed) {
    ...
    const created = await prisma.taskTemplate.create({
      data: {
        title: seed.title,
        description:
          'Tarefa importada do planejamento real CIPAVD/SMIF para centralização do acompanhamento no sistema.',
        phaseId: phase.id,
    ...
    for (const templateSeed of templatesSeed) {
      ...
      await prisma.taskInstance.create({
        data: {
          ...
          localityId: locality.id,
          dueDate,
          status: done ? TaskStatus.DONE : TaskStatus.NOT_STARTED,
  ...
  // Backlog de evolução para desligamento definitivo do Google Drive
  const backlogSeeds = [
    {
      title: 'Evolução do Acervo: OCR e indexação textual dos documentos',
      description:
        'Implementar OCR e indexação de conteúdo para busca por texto dentro de PDF/Word e histórico de versões.',
      dueDate: new Date('2026-04-30T00:00:00.000Z'),
    },
    {
      title:
        'Evolução do Acervo: versionamento e trilha de alteração de arquivos',
      description:
        'Permitir controle de versões por documento, comparação de versões e restauração.',
      dueDate: new Date('2026-05-15T00:00:00.000Z'),
    },
    {
      title: 'Evolução do Acervo: fluxo formal de aprovação/publicação',
      description:
        'Adicionar workflow de rascunho, revisão e publicação oficial para documentos sensíveis.',
      dueDate: new Date('2026-05-31T00:00:00.000Z'),
    },
    {
      title:
        'Evolução do Acervo: assinatura digital com certificado ICP-Brasil',
      description:
        'Integrar assinatura eletrônica avançada para documentos oficiais com validação jurídica reforçada.',
      dueDate: new Date('2026-06-30T00:00:00.000Z'),
    },
    {
      title: 'Evolução do Acervo: política de retenção e descarte documental',
      description:
        'Implementar prazos de guarda, arquivamento e descarte para eliminar dependência operacional do Drive.',
    ...
    for (const seed of backlogSeeds) {
      const backlogTemplate = await prisma.taskTemplate.create({
        data: {
          ...
          phaseId: phaseByName.get(PhaseName.ACOMPANHAMENTO)!.id,
          appliesToAllLocalities: false,
          reportRequiredDefault: false,
        },
      });
...
async function importDocuments(
  localityByCode: Map<string, { id: string; code: string; name: string }>,
  meetingId: string,
  activitiesByCode: Map<string, { id: string; localityId: string | null }>,
) {
  ...
  for (const absPath of files) {
    ...
    const storageKey = `${Date.now()}-${crypto.randomUUID()}${path.extname(absPath).toLowerCase()}`;
    ...
    let linkMeetingId: string | null = null;
    let linkActivityId: string | null = null;
...
function extractDateToken(raw: string, fallbackYear = 2026) {
...
function inferLocalityCodeFromText(
  sourcePath: string,
  text: string,
  localityByCode: Map<string, { id: string; code: string; name: string }>,
) {
  ...
  for (const locality of localityByCode.values()) {
    ...
    if (city && textNorm.includes(city)) return locality.code;
  }
...
async function ensureEloRoles() {
  const seeds: Array<{ code: string; name: string; sortOrder: number }> = [
...
function extractTextByFile(absPath: string) {
  const ext = path.extname(absPath).toLowerCase();
  if (ext === '.docx') {
    const lines = readDocxLines(absPath);
    const text = lines.join('\n').trim();
    return {
      status: text ? DocumentParseStatus.EXTRACTED : DocumentParseStatus.FAILED,
      text,
  ...
  return {
    status: DocumentParseStatus.NOT_SUPPORTED,
    text: '',
    metadata: { extractor: 'binary-not-supported' },
  };
}

function parseEloCandidates(text: string) {
  const chunks = text
    .split(/[\n|]+/)
    .map((part) => part.trim())
    .filter(Boolean);
  ...
  let currentRole: string = 'CPCA';
  for (const chunk of chunks) {
    const norm = normalize(chunk);
    if (norm.includes('PSICOLOG')) currentRole = 'PSICOLOGIA';
    else if (norm.includes('JURID')) currentRole = 'JURIDICO';
    else if (
      norm.includes('SERVICO SOCIAL') ||
      norm.includes('ASSISTENCIA SOCIAL')
    )
      currentRole = 'SSO';
    else if (norm.includes('CPCA')) currentRole = 'CPCA';
    else if (
      norm.includes('GRADUADO-MASTER') ||
      norm.includes('GRADUADO MASTER')
    )
      currentRole = 'GRAD_MASTER';
    else if (norm.includes('DOUTRINA')) currentRole = 'DOUTRINA';

    const nameMatch = chunk.match(
      /\b(MJ|MAJ|CAP|TEN|1T|2T|1S|2S|3S|SO|SGT)\s+([A-Za-zÀ-ÿ'.-]+(?:\s+[A-Za-zÀ-ÿ'.-]+){0,5})/i,
    );
    ...
    if (invalidWords.some((w) => normName.includes(w))) continue;
    if (name.split(' ').length < 2) continue;

    const phone =
      chunk.match(/\(\d{2}\)\s*\d{4,5}-?\d{4}/)?.[0] ??
...
function parseMonthDueDate(raw: string) {
  const norm = normalize(raw);
  if (norm.includes('FEV')) return new Date(Date.UTC(2026, 1, 20));
  if (norm.includes('MAR')) return new Date(Date.UTC(2026, 2, 20));
  if (norm.includes('ABR')) return new Date(Date.UTC(2026, 3, 20));
  if (norm.includes('MAI')) return new Date(Date.UTC(2026, 4, 20));
  if (norm.includes('JUN')) return new Date(Date.UTC(2026, 5, 20));
  return null;
}

async function syncOperationalDataFromDocuments(
  localityByCode: Map<string, { id: string; code: string; name: string }>,
) {
  ...
  for (const doc of docs) {
    const absPath = path.join(repoRoot, doc.sourcePath);
    ...
    if (extraction.status === DocumentParseStatus.EXTRACTED) {
    ...
    if (doc.localityId) {
      await linkDocumentEntity(
        doc.id,
        DocumentLinkEntity.LOCALITY,
        doc.localityId,
    ...
    const localityCode = inferLocalityCodeFromText(
      doc.sourcePath,
      text,
      localityByCode,
    );
    ...
    if (
      sourceNorm.includes('REUNIO') ||
      sourceNorm.includes('PAUTA DA REUNIAO')
    ) {
      const dateByPath =
        extractDateToken(doc.sourcePath, 2026) ??
        extractDateToken(
          text.match(/DIA\s+\d{2}[./]\d{2}/i)?.[0] ?? '',
          2026,
        ) ??
      ...
      if (!existing) summary.createdMeetings += 1;
      await linkDocumentEntity(
        doc.id,
        DocumentLinkEntity.MEETING,
        meeting.id,
        'Reunião extraída do documento',
      );
      summary.linkedEntities += 1;
      ...
      if (sourceNorm.includes('PAUTA DA REUNIAO')) {
        const milestones = [
          ...text.matchAll(
            /(FEV(?:EREIRO)?|MAR[ÇC]O|ABRIL(?:\/JUN)?|MAIO|JUNHO)\s*:\s*\d+\.\s*([^;.\n]+)/gi,
          ),
        ];
        for (const milestone of milestones) {
          const title = `Pauta 03.02: ${milestone[2].trim().replace(/\s+/g, ' ')}`;
          const dueDate =
            parseMonthDueDate(milestone[1]) ?? new Date(Date.UTC(2026, 1, 20));
          const phaseName =
            dueDate.getUTCMonth() <= 1
              ? PhaseName.PREPARACAO
              : dueDate.getUTCMonth() <= 4
                ? PhaseName.EXECUCAO
                : PhaseName.ACOMPANHAMENTO;
          const phase = phaseByName.get(phaseName);
          const br = localityByCode.get('BR');
          if (!phase || !br) continue;
          ...
          if (!template) {
            template = await prisma.taskTemplate.create({
              data: {
                title,
                description:
                  'Item de execução importado da pauta de reunião do repositório oficial.',
                phaseId: phase.id,
                appliesToAllLocalities: false,
                reportRequiredDefault: false,
              },
    ...
    if (
      sourceNorm.includes('7. CIPAVD-SMIF/RESPOSTAS DOS GSD') &&
      ...
      if (locality && eloCandidates.length) {
        for (const candidate of eloCandidates) {
          if (!candidate.phone && !sourceNorm.includes('ELOS')) continue;
          const role =
            eloRoleByCode.get(candidate.roleCode) ?? eloRoleByCode.get('CPCA');
          if (!role) continue;
          ...
          if (!existing) summary.createdElos += 1;
          await linkDocumentEntity(
            doc.id,
            DocumentLinkEntity.ELO,
            elo.id,
            'Elo extraído do documento',
          );
          summary.linkedEntities += 1;
        }
      }
    ...
    if (
      sourceNorm.includes('ATIVIDADES CIPAVD 2026') ||
      ...
      const ranges = [
        ...flattened.matchAll(
          /([A-ZÁÀÂÃÉÊÍÓÔÕÚÇ][A-Za-zÀ-ÿ\s]+-\s*[A-Z]{2})[\s\S]{0,30}?(\d{2}[./]\d{2})\s*a\s*(\d{2}[./]\d{2})/g,
        ),
      ];
      ...
      for (const range of ranges) {
        ...
        const activity =
          ...
          (await prisma.activity.create({
            data: {
              title,
              description:
                'Atividade extraída automaticamente do cronograma oficial do repositório CIPAVD.',
              localityId: mappedLocality?.id ?? null,
              eventDate: start,
              reportRequired: true,
        ...
        if (!existing) summary.createdActivities += 1;
        await linkDocumentEntity(
          doc.id,
          DocumentLinkEntity.ACTIVITY,
          activity.id,
          'Atividade extraída do cronograma',
        );
        summary.linkedEntities += 1;
      }
    }
    ...
    if (
      sourceNorm.includes('ATIVIDADES DA CIPAVD NO SMIF') &&
      ...
      const tokens = text
        ...
        .filter(Boolean);
      const br = localityByCode.get('BR');
      const phasePrep = phaseByName.get(PhaseName.PREPARACAO);
      if (br && phasePrep) {
        for (let i = 0; i < tokens.length - 4; i += 1) {
          ...
          if (!template) {
            template = await prisma.taskTemplate.create({
              data: {
                title,
                description:
                  'Tarefa de execução interna extraída do plano de equipe da CIPAVD.',
            ...
            await linkDocumentEntity(
              doc.id,
              DocumentLinkEntity.TASK_TEMPLATE,
              template.id,
              'Template extraído do plano de equipe',
            );
            summary.linkedEntities += 1;
          }
          ...
          if (!existingInstance) {
            ...
            await linkDocumentEntity(
              ...
              DocumentLinkEntity.TASK_INSTANCE,
              task.id,
              'Tarefa extraída do plano de equipe',
            );
            summary.linkedEntities += 1;
          }
        }
  ...
  const notSupportedRow = parseStatusCounts.find(
    (row) => row.parseStatus === DocumentParseStatus.NOT_SUPPORTED,
  );
  const notSupportedCount = Number((notSupportedRow?._count as any)?._all ?? 0);
  const br = localityByCode.get('BR');
  const phaseFollowUp = phaseByName.get(PhaseName.ACOMPANHAMENTO);
  if (br && phaseFollowUp) {
...
async function main() {
  ...
  const operationalSync =
    await syncOperationalDataFromDocuments(localityByCode);
  ...
  const outFile = path.join(rootDir, 'docs', 'migration-summary.json');
