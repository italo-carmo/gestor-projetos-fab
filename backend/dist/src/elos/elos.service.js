...
let ElosService = class ElosService {
    ...
    async listCommissionMembers(filters, _user) {
    ...
    async listCommissionCandidates(filters, _user) {
        const commissionRole = await this.getCommissionRole();
        if (!commissionRole)
            return { items: [] };
        const where = {
    ...
    async updateCommissionMember(payload, user) {
        ...
        const updated = await this.prisma.user.update({
            where: { id: userId },
            data: {
                commissionFunction: payload.functionText === undefined
                    ? undefined
                    : payload.functionText && payload.functionText.trim()
                        ? (0, sanitize_1.sanitizeText)(payload.functionText.trim())
                        : null,
                commissionPhone: payload.phone === undefined
                    ? undefined
                    : this.maskCommissionPhone(payload.phone),
                commissionSeniority: payload.seniority === undefined
                    ? undefined
    ...
    async reorderCommissionMembers(payload, user) {
        ...
        if (!normalizedIds.length) {
            (0, http_error_1.throwError)('VALIDATION_ERROR', { reason: 'ORDER_REQUIRED' });
        }
        ...
        if (normalizedIds.length !== currentMembers.length) {
            (0, http_error_1.throwError)('VALIDATION_ERROR', { reason: 'ORDER_INCOMPLETE' });
        }
        const currentSet = new Set(currentMembers.map((item) => item.id));
        const hasUnknown = normalizedIds.some((id) => !currentSet.has(id));
        if (hasUnknown) {
            (0, http_error_1.throwError)('VALIDATION_ERROR', { reason: 'ORDER_HAS_INVALID_MEMBER' });
        }
        await this.prisma.$transaction(normalizedIds.map((id, index) => this.prisma.user.update({
            where: { id },
