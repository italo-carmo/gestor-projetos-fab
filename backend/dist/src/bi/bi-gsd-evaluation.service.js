...
exports.BiGsdEvaluationService = void 0;
...
let BiGsdEvaluationService = class BiGsdEvaluationService {
    prisma;
    constructor(prisma) {
        this.prisma = prisma;
    }
    async importResponses(file, user, options = {}) {
        ...
        const responseModel = this.prisma.biGsdEvaluationResponse;
        const importModel = this.prisma.biGsdEvaluationImportBatch;
        if (replaceAll) {
    ...
    async listCardSettings() {
        const cardSettingModel = this.prisma.biGsdEvaluationCardSetting;
    ...
    async dashboard(filters) {
        ...
        const categoricalDistributions = categoricalColumns.map((column) => {
            const distribution = this.buildDistribution(filteredRows, column, localityKey);
            return {
                key: column.key,
                label: column.label,
                type: column.type,
    ...
    buildDistribution(rows, column, localityKey) {
        const counter = new Map();
        const localitiesByOption = new Map();
        for (const row of rows) {
            ...
            if (column.type === 'MULTI_SELECT') {
                for (const token of this.splitMultiValues(raw, true)) {
                    ...
                    if (localityValue) {
                        const localities = localitiesByOption.get(token) ?? new Set();
            ...
            if (localityValue) {
                const localities = localitiesByOption.get(raw) ?? new Set();
                localities.add(localityValue);
        ...
        const data = Array.from(counter.entries())
            ...
            localities: Array.from(localitiesByOption.get(label) ?? []).sort((a, b) => a.localeCompare(b, 'pt-BR')),
        }))
    ...
    detectObservedLocalityKey(columns) {
        let bestKey = null;
        let bestScore = 0;
        for (const column of columns) {
            ...
            let score = 0;
            if (combined.includes('OMOBSERVADA'))
                score = 100;
            else if (keyNorm === 'OM' || labelNorm === 'OM')
                score = 80;
            else if (combined.includes('ORGANIZACAOMILITAR'))
                score = 60;
            else if (combined.includes('LOCALIDADE'))
                score = 50;
            else if (combined.includes('UNIDADE'))
                score = 40;
            if (score > bestScore) {
                bestScore = score;
                bestKey = column.key;
...
exports.BiGsdEvaluationService = BiGsdEvaluationService;
exports.BiGsdEvaluationService = BiGsdEvaluationService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_service_1.PrismaService])
], BiGsdEvaluationService);
//# sourceMappingURL=bi-gsd-evaluation.service.js.map