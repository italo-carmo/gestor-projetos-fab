...
exports.StrategicService = void 0;
...
function tokenizeAndCount(texts, minLen = 3) {
    const freq = new Map();
...
let StrategicService = class StrategicService {
    prisma;
    litellm;
    ...
    async situationalDashboard() {
        const [surveyData, domesticViolenceData, recruitsData, complaintsData, activitiesData, missionsData, localities,] = await Promise.all([
            this.getSurveyKpis(),
    ...
    async geoMap() {
        ...
        const [complaints, activities, missions] = await Promise.all([
            this.prisma.cpcComplaintCase.findMany({
                select: {
        ...
        const ufMap = new Map();
        const locUfMap = new Map();
        for (const loc of localities) {
            if (loc.uf)
                locUfMap.set(loc.id, loc.uf);
        }
        const ensureUf = (uf) => {
            if (!ufMap.has(uf)) {
                ufMap.set(uf, {
    ...
    async aggressorProfile() {
        ...
        const moralCases = cases.filter((c) => c.complaintType === 'MORAL');
        const sexualCases = cases.filter((c) => c.complaintType === 'SEXUAL');
        const hierarchicalCount = cases.filter((c) => c.hierarchicalFunctionalRelation &&
            /superior|chefia|comando|hierarq/i.test(c.hierarchicalFunctionalRelation)).length;
        ...
        return {
            ...
            byLocality: countByField(cases, 'localityId').map((item) => {
                const loc = cases.find((c) => c.localityId === item.label)?.locality;
                return { ...item, localityCode: loc?.code ?? '', localityName: loc?.name ?? item.label };
            }),
        };
    }
    async textAnalysis() {
        const [recruitsResponses, activityReports, bestPracticeCycleResponses, cpcaComments,] = await Promise.all([
            ...
            this.prisma.biBestPracticeCycleResponse.findMany({
                select: { interactionDifferenceComment: true },
            }).catch(() => []),
            this.prisma.cpcComplaintComment.findMany({
                select: { text: true },
            }).catch(() => []),
        ]);
        const filterFreeText = (texts) => {
            const freq = new Map();
            for (const t of texts)
                freq.set(t, (freq.get(t) ?? 0) + 1);
            return texts.filter((t) => freq.get(t) <= 3 && t.trim().length > 5);
        };
        const suggestionTexts = filterFreeText(recruitsResponses.map((r) => r.suggestionComment).filter(Boolean));
        const reportObservations = activityReports
            .map((r) => r.mainPointsObserved)
            .filter(Boolean);
        const reportAttention = activityReports
            .map((r) => r.attentionPoints)
            .filter(Boolean);
        const reportConclusions = activityReports
            .map((r) => r.conclusion)
            .filter(Boolean);
        const bestPracticeComments = bestPracticeCycleResponses
            .map((r) => r.interactionDifferenceComment)
            .filter(Boolean);
    ...
    async aiSourceReferences(type) {
        const refs = [];
        const seen = new Set();
        const pushRef = (id, label, href, description) => {
            const safeHref = String(href || '').trim();
            if (!safeHref || seen.has(safeHref))
                return;
            seen.add(safeHref);
            refs.push({
                id,
        ...
        const includePanels = type === 'executive' ||
            type === 'situational' ||
            type === 'aggressor' ||
            type === 'text';
        if (includePanels) {
            pushRef('bi-survey-dashboard', 'Pesquisa institucional (BI Survey)', '/dashboard/bi');
            pushRef('bi-domestic-dashboard', 'Pesquisa de violência doméstica', '/dashboard/bi-violencia-domestica');
            pushRef('bi-recruits-dashboard', 'Pesquisa de recrutas', '/dashboard/bi-recrutas');
        }
        if (type === 'executive' || type === 'situational' || type === 'text') {
            const latestRecruitBatch = await this.prisma.biRecruitsImportBatch
                .findFirst({
            ...
            if (latestRecruitBatch) {
                pushRef(`recruits-batch-${latestRecruitBatch.id}`, `Lote de recrutas: ${latestRecruitBatch.fileName || latestRecruitBatch.id}`, '/dashboard/bi-recrutas', `Importado em ${fmtDate(latestRecruitBatch.importedAt)} (${Number(latestRecruitBatch.insertedRows ?? 0)} linha(s))`);
            }
        }
        if (type === 'executive' || type === 'situational' || type === 'text') {
            const recentReports = await this.prisma.activityReport
                .findMany({
                take: 3,
            ...
            for (const report of recentReports) {
                ...
                pushRef(`activity-report-${report?.id ?? activityId}`, `Relatório de atividade: ${title}`, href, refDate ? `Última atualização em ${refDate}` : undefined);
            }
        }
        if (type === 'executive' || type === 'situational' || type === 'geo') {
            const recentMissions = await this.prisma.mission
                .findMany({
                take: 3,
            ...
            for (const mission of recentMissions) {
                ...
                pushRef(`mission-${missionId}`, `Missão: ${title}`, href, period ? `Período ${period}` : undefined);
            }
        }
        if (type === 'executive' || type === 'aggressor' || type === 'situational') {
            const recentCases = await this.prisma.cpcComplaintCase
                .findMany({
                take: 3,
            ...
            for (const item of recentCases) {
                ...
                pushRef(`complaint-${item?.id ?? caseNumber}`, `Denúncia ${caseNumber}`, href, [workflow, status, date].filter(Boolean).join(' • '));
            }
        }
        pushRef('strategic-dashboard', 'Painel estratégico consolidado', '/dashboard/estrategico');
        return refs.slice(0, 12);
    }
    async strategicAiNarrative() {
        if (!this.litellm.isConfigured()) {
            throw new common_1.ServiceUnavailableException(`LiteLLM não configurado. No servidor, edite o .env do backend (ex.: /opt/gestao-projetos/backend/.env) e defina ` +
                `chave (${litellm_service_1.LITELLM_API_KEY_ENV_KEYS.join(' ou ')}) e URL (${litellm_service_1.LITELLM_BASE_URL_ENV_KEYS.join(' ou ')}). ` +
        ...
        const compactGeo = {
            statesSample: (geo.states ?? []).slice(0, 12).map((s) => ({
                uf: s.uf,
        ...
        const system = 'Você é analista institucional da FAB (CIPAVD/SMIF). ' +
        ...
        try {
            const { content, model } = await this.litellm.chatCompletion({
                messages: [
                    ...
                    {
                        role: 'user',
                        content: 'Com base exclusivamente nos dados JSON abaixo, redija um resumo executivo para comando.\n\n' +
                            payloadJson,
                    },
                ],
                max_tokens: 2500,
            ...
            throw new common_1.ServiceUnavailableException(`Falha ao gerar narrativa via LiteLLM: ${msg}`);
        }
    }
    async executiveReportPdf() {
        ...
        return new Promise((resolve, reject) => {
            ...
            const progressBar = (x, y, w, pct, color, label, valueText) => {
            ...
            const rankingRow = (x, y, w, rank, label, count, pct, color, maxCount) => {
            ...
            doc.y = 90;
            doc.fontSize(9).fillColor(GRAY).text(`Gerado em ${new Date().toLocaleDateString('pt-BR')} às ${new Date().toLocaleTimeString('pt-BR')}`, { align: 'right' });
            doc.moveDown(0.8);
            ...
            if (profile.totalCases > 0) {
                ...
                doc.fontSize(8).fillColor(DARK).text(`${hierPct}% dos casos (${hierCount}) envolvem relação hierárquica superior-subordinado. ` +
                    ...
                    `(${Math.max(profile.byComplaintType?.moral?.percent ?? 0, profile.byComplaintType?.sexual?.percent ?? 0)}%).`, LEFT + 52, insY + 6, { width: PAGE_W - 68 });
                doc.y = insY + 48;
            }
            ensureSpace(100);
            ...
            if (topWords.length > 0) {
                ...
                if (sourcesWithData.length > 0) {
                    ...
                    for (const [idx, src] of sourcesWithData.entries()) {
                        ...
                        doc.fontSize(6).fillColor(GRAY).text(src.topWords.slice(0, 5).map((w2) => w2.word).join(', '), sx + 6, sy + 34, { width: srcColW - 18 });
                        sx += srcColW;
                    }
                    doc.y = sy + 58;
                }
            ...
            const statesWithData = (geoData.states ?? []).filter((s) => s.complaints + s.activities + s.missions > 0);
            if (statesWithData.length > 0) {
                ensureSpace(120);
                sectionHeader('07', 'DISTRIBUIÇÃO GEOGRÁFICA');
                doc.moveDown(0.3);
                doc.fontSize(8).fillColor(GRAY).text(`${statesWithData.length} estado(s) com registros de ${geoData.totalLocalitiesWithUf} localidades com UF preenchida.`);
                doc.moveDown(0.3);
                ...
                for (const [ri, s] of statesWithData.slice(0, 15).entries()) {
                    ...
                    const rowY = doc.y;
                    if (ri % 2 === 0)
                        doc.rect(LEFT, rowY, PAGE_W, 13).fill(BG_LIGHT);
                    let rx = LEFT;
            ...
            doc.moveDown(0.3);
            doc.fontSize(7).fillColor(GRAY).text('DOCUMENTO CLASSIFICADO — USO INTERNO  |  Sistema de Gestão CIPAVD/SMIF  |  Força Aérea Brasileira', { align: 'center' });
            doc.fontSize(6).fillColor('#999999').text(`Gerado automaticamente em ${new Date().toLocaleDateString('pt-BR')} às ${new Date().toLocaleTimeString('pt-BR')}. Dados sujeitos a atualização.`, { align: 'center' });
            doc.end();
        });
    }
    async getSurveyKpis() {
        try {
            const model = this.prisma.biSurveyResponse;
    ...
    async getDomesticViolenceKpis() {
        try {
            const model = this.prisma.biDomesticViolenceResponse;
            const total = await model.count();
            if (total === 0)
    ...
    async getRecruitsKpis() {
        try {
            const model = this.prisma.biRecruitsResponse;
            const total = await model.count();
            if (total === 0)
            ...
            const safeCount = all.filter((r) => r.willingnessReport === 'Seguro(a)').length;
            const knowProcess = all.filter((r) => r.knowReportProcess === 'Sim').length;
            return {
                totalResponses: total,
                safeCount,
    ...
    async getComplaintsKpis() {
        try {
            const model = this.prisma.cpcComplaintCase;
            const total = await model.count();
    ...
    async getActivitiesKpis() {
        try {
            ...
            const withReport = await this.prisma.activityReport.count();
            const signed = await this.prisma.activityReport.count({ where: { signedAt: { not: null } } });
    ...
    async getMissionsKpis() {
        try {
            const model = this.prisma.mission;
            const total = await model.count();
...
exports.StrategicService = StrategicService;
exports.StrategicService = StrategicService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_service_1.PrismaService,
        litellm_service_1.LitellmService])
], StrategicService);
//# sourceMappingURL=strategic.service.js.map