...
function normalizeToken(token, isFirstWord, acronyms) {
    if (!token)
        return token;
    ...
    if (acronyms.has(upper))
        return upper;
    if (ROMAN_NUMERALS.test(upper))
        return upper;
    if (!isFirstWord && PT_PREPOSITIONS.has(lower))
        return lower;
    return capitalizeWord(token);
}
function normalizeLabel(value, acronyms) {
    if (!value)
        return value;
...
async function normalizeUsers(acronyms) {
...
async function normalizeLocalities(acronyms) {
...
async function normalizeSpecialties(acronyms) {
...
async function normalizeEloRoles(acronyms) {
...
async function normalizeElos(acronyms) {
    ...
    for (const elo of elos) {
        ...
        if (nextName === elo.name && nextOm === elo.om)
            continue;
...
async function normalizePostos(acronyms) {
    ...
    for (const posto of postos) {
        ...
        if (!nextName || nextName === posto.name)
            continue;
...
//# sourceMappingURL=normalize-names.js.map