...
const REPORT_SECTION_LABELS: Record<AssistantReportSectionId, string> = {
  ...
  CPCA_CASES: 'Reportes CPCA',
  SMIF_CASES: 'Reportes SMIF',
...
const REPORT_SECTION_DESCRIPTION: Record<AssistantReportSectionId, string> = {
  ...
  CPCA_CASES:
    'Apresenta volume, status e sinais principais dos reportes CPCA.',
  SMIF_CASES:
  ...
  private async buildContext(
    ...
    if (kind === 'MISSION') {
      ...
      sectionFacts.CPCA_CASES = {
        ...
        facts: [
          cpcaCases.length
            ? `${cpcaCases.length} reporte(s) CPCA/SMIF foram localizados no mesmo recorte geográfico.`
            : 'Nenhum reporte CPCA/SMIF foi localizado no recorte desta missão.',
      ...
      sectionFacts.SMIF_CASES = {
        ...
        facts: [
          smifCases.length
            ? `${smifCases.length} reporte(s) SMIF foram localizados para a localidade da missão.`
            : 'Nenhum reporte SMIF foi localizado para a localidade da missão.',
        ],
        chartIds: [],
        tableIds: [],
        imageIds: [],
    ...
    sectionFacts.STRATEGIC_OVERVIEW = {
      ...
      facts: [
        ...
        `${situational.complaints.totalCases} reporte(s) formal(is) no consolidado estratégico.`,
    ...
    sectionFacts.RECOMMENDATIONS = {
      ...
      facts: [
        'As recomendações devem refletir o cruzamento entre pesquisas, reportes e presença operacional.',
      ],
      chartIds: [],
      tableIds: [],
      imageIds: [],
