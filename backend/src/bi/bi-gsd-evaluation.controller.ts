...
@Controller('bi/gsd-evaluation')
@UseGuards(JwtAuthGuard, RbacGuard)
export class BiGsdEvaluationController {
  constructor(
    private readonly biGsdEvaluation: BiGsdEvaluationService,
  ...
  async dashboardPdf(
    ...
    const buffer = await this.biPdf.gsdEvaluationDashboardPdf({
      from,
      to,
      q,
      combineMode,
    ...
    const filename = `bi-avaliacao-gsd-${new Date().toISOString().slice(0, 10)}.pdf`;
    res.set({
