...
export function MilitaryHighlightsPanel() {
  ...
  const openCardEditor = useCallback(() => {
    ...
    setCardEditorDraft({
      ...
      innerBackground:
        s.highlightsInnerBackground?.trim() || PICKER_DEFAULT_INNER_BG,
      innerTitleColor:
        s.highlightsInnerTitleColor?.trim() || DEFAULT_INNER_TITLE_COLOR,
      innerBodyColor:
        s.highlightsInnerBodyColor?.trim() || DEFAULT_INNER_BODY_COLOR,
    });
    ...
    () =>
      [
        ...((highlightsQuery.data?.items ??
          []) as SocialCommunicationHighlight[]),
      ].sort((a, b) => {
  ...
  const renderHighlightCards = (
    ...
    const highlightChipSx = {
      bgcolor: hexToRgba(theme.titleColor, 0.08),
                ...
                <Stack direction="row" spacing={1.2} alignItems="stretch">
                  ...
                  <Stack sx={{ minWidth: 0, flex: 1 }}>
                    <Typography
                      variant="subtitle1"
                      fontWeight={800}
                      sx={{ color: theme.titleColor, mb: 0.55 }}
                    >
                      {item.militaryName}
                    </Typography>
                    <Stack
      ...
      >
        <CardContent sx={{ py: 2.2 }}>
          ...
          {highlightsQuery.isLoading ? (
            ...
            <EmptyState
              title="Sem destaques"
              description="Nenhum destaque cadastrado."
            />
          ) : (
