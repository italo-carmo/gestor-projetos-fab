...
import { useComgepSituationRoom } from "../../api/hooks";
...
function buildOmReason(item: any) {
  ...
  if (Number(complaints?.openCases ?? 0) > 0) {
    reasons.push(`${complaints.openCases} reporte(s) aberto(s)`);
  }
  ...
  if (Number(item?.surveyRate ?? 0) >= 20) {
    reasons.push(
      `pesquisa de escolas em ${Number(item.surveyRate).toFixed(1)}%`,
    );
  }
  return (
    reasons.slice(0, 3).join(" • ") ||
    "Risco composto por reportes, cobertura e sinais BI."
  );
}
...
function underreportChipColor(item: any) {
  if (!item?.underreport?.eligible) return "default";
  return subnotificationColor(Number(item?.underreport?.percent ?? 0));
}
...
function formatUnderreport(item: any) {
  const underreport = item?.underreport ?? {};
  if (underreport.eligible) return formatPercent(underreport.percent);
  if (underreport.status === "NO_RESEARCH") return "Sem pesquisa";
  if (underreport.status === "INSUFFICIENT_RESEARCH") return "Base baixa";
  return "N/A";
}

function underreportTooltip(item: any) {
  const underreport = item?.underreport ?? {};
  if (underreport.eligible) {
    return `Estimativa em ${underreport.basis ?? "recorte"} com n=${underreport.researchTotal ?? 0}.`;
  }
  if (underreport.status === "NO_RESEARCH") {
    return "Não há pesquisa normalizada neste recorte; não estimar subnotificação.";
  }
  if (underreport.status === "INSUFFICIENT_RESEARCH") {
    return `Base de pesquisa pequena (n=${underreport.researchTotal ?? 0}); usar apenas como sinal auxiliar.`;
  }
...
function SignalsPreview({ item }: { item: any }) {
  ...
  const chips = [
    ...
    military > 0
      ? { label: `Autor militar ${military}`, color: "#2E7D32" }
      : null,
...
export function ComgepStrategicTab() {
  ...
  const [detailModal, setDetailModal] = useState<
    ...
    | "riskOm"
  >(null);
  const [selectedPriorityUf, setSelectedPriorityUf] = useState<any | null>(
    null,
  );
  ...
  const confidencePercent = Number(
    dataConfidence.supportedCoveragePercent ?? 0,
  );
  const omRiskIndex = Array.isArray(room?.details?.omRiskIndex)
    ? room.details.omRiskIndex
    : [];
  const priorityUfs = Array.isArray(room?.details?.ufMatrix)
    ? room.details.ufMatrix
    : [];
  ...
  const retaliationOms = useMemo(
    () =>
      omRiskIndex
        .filter(
          (item: any) => Number(item?.complaints?.retaliationCases ?? 0) > 0,
        )
        .sort(
          (a: any, b: any) =>
  ...
  const retaliationCount = retaliationOms.reduce(
    (sum: number, item: any) =>
      sum + Number(item?.complaints?.retaliationCases ?? 0),
    0,
  );
  const stalledCount = stalledOms.reduce(
    (sum: number, item: any) =>
      sum + Number(item?.complaints?.stalledCases ?? 0),
    0,
  );
  ...
  return (
    <Stack spacing={2}>
      ...
      <Grid container spacing={2}>
        ...
        <Grid size={{ xs: 12, md: 6, xl: 3 }}>
          <SummaryCard
            title="Reporte sem pesquisa"
            value={summary.complaintOmsWithoutResearchCount ?? 0}
            subtitle="OMs com reporte formal sem base de pesquisa comparável na mesma OM."
            color="#4E342E"
            icon={<HubRoundedIcon />}
      ...
      <SectionCard
        ...
        subtitle="UFs priorizadas por reporte formal, sinais de pesquisa com base mínima, cobertura CPCA e resposta operacional. Clique na linha para entender a posição no ranking."
      >
        <Typography
          variant="caption"
          color="text.secondary"
        ...
        >
          Pesquisas de escolas e violência doméstica são exibidas separadamente;
          missão entra como presença inicial amortecida, não como previsão.
        </Typography>
        <TableContainer>
          <Table size="small">
            <TableHead>
              <TableRow>
                ...
                <TableCell align="right">
                  <strong>Resposta em campo</strong>
                </TableCell>
                <TableCell>
            ...
            <TableBody>
              {priorityUfs.slice(0, 12).map((item: any, index: number) => (
                ...
                >
                  ...
                  <TableCell align="right">
                    {item.complaints?.openCases ?? 0} /{" "}
                    {item.complaints?.totalCases ?? 0}
                  </TableCell>
                  <TableCell align="right">
                  ...
                  <TableCell align="right">
                    <Tooltip title={underreportTooltip(item)} arrow>
                      <Chip
                        size="small"
                        color={underreportChipColor(item)}
                        label={formatUnderreport(item)}
                      />
                    </Tooltip>
                  </TableCell>
                  <TableCell align="right">
      ...
      >
        <Typography
          ...
          sx={{ display: "block", mb: 1.2 }}
        >
          O ranking prioriza reporte formal e proteção, usa pesquisas com
            ...
            <TableBody>
              {topRiskOms.slice(0, 12).map((item: any, index: number) => (
                ...
                >
                  ...
                  <TableCell>
                    {item.recommendedAction ?? buildOmReason(item)}
                  </TableCell>
                </TableRow>
              ))}
              {!topRiskOms.length && (
        ...
        <Stack spacing={1.25}>
          <AiCopilotCtaRow
            ...
            {...buildComgepCopilotLinks({
              ...
              description:
                "Casos abertos com risco de retaliação e OMs impactadas no recorte atual.",
            })}
          />
          <MeaningBlock
            title="O que isso significa"
            meaning="Este indicador mostra quantos casos abertos hoje carregam marcação de retaliação ou risco de retaliação. Ele não mede volume total de reportes; ele destaca os casos mais sensíveis para proteção da vítima e ação imediata do gestor."
            source="Fonte: reportes abertos da base CPCA/SMIF consolidados na Sala COMGEP."
          />
          ...
          >
            <Stack spacing={1}>
              <ModalRow
                label="Casos com risco de retaliação"
                value={retaliationCount}
                color="#D32F2F"
              />
              ...
              >
                ...
                reportes existem, mas quantos casos demandam contenção de
                ...
                <TableBody>
                  {retaliationOms.slice(0, 12).map((item: any) => (
                    <TableRow key={item.id}>
                      ...
                      <TableCell>{item.uf || "—"}</TableCell>
                      <TableCell align="right">
                        {item.complaints?.retaliationCases ?? 0}
                      </TableCell>
                      <TableCell align="right">
                        {item.complaints?.openCases ?? 0}
                      </TableCell>
        ...
        <Stack spacing={1.25}>
          <AiCopilotCtaRow
            ...
            {...buildComgepCopilotLinks({
              ...
              description:
                "Casos abertos há mais de 30 dias e OMs com atraso relevante de tratamento.",
            })}
          />
          <MeaningBlock
            title="O que isso significa"
            ...
            source="Fonte: reportes abertos da base CPCA/SMIF, com cálculo de tempo desde o registro."
          />
          <ComgepAccordionSection
            title="Resumo executivo"
          ...
          >
            <Stack spacing={1}>
              ...
              <Typography
                ...
                sx={{ lineHeight: 1.7 }}
              >
                Este número ajuda a separar carga de trabalho normal de passivo
                que já pressiona a confiança no fluxo institucional. Quanto mais
                casos estagnados, maior o risco de desgaste e agravamento.
              </Typography>
            </Stack>
          </ComgepAccordionSection>
          <ComgepAccordionSection
                ...
                <TableBody>
                  {stalledOms.slice(0, 12).map((item: any) => (
                    <TableRow key={item.id}>
                      ...
                      <TableCell>{item.uf || "—"}</TableCell>
                      <TableCell align="right">
                        {item.complaints?.stalledCases ?? 0}
                      </TableCell>
                      <TableCell align="right">
                        {item.complaints?.openCases ?? 0}
                      </TableCell>
        ...
        <Stack spacing={1.25}>
          <AiCopilotCtaRow
            ...
            {...buildComgepCopilotLinks({
              label: "UFs críticas",
              description:
                "UFs classificadas em faixa crítica pela matriz da Sala COMGEP.",
              focus: { kind: "kpi_critical_ufs" },
            })}
          />
          <MeaningBlock
            title="O que isso significa"
            meaning="Este KPI conta UFs em faixa crítica na matriz COMGEP. A classificação separa reporte formal, pesquisas de escolas, violência doméstica, cobertura CPCA e resposta operacional; missões são amortecidas porque a série histórica ainda é inicial."
            source="Fonte: matriz consolidada da Sala COMGEP, com cruzamento controlado entre reportes CPCA/SMIF, BI normalizado e presença operacional."
          />
          <ComgepAccordionSection
            title="Resumo executivo"
          ...
          >
            <Stack spacing={1}>
              ...
              <Typography
                ...
                sx={{ lineHeight: 1.7 }}
              >
                Esta leitura já combina risco, governança e presença. A lista
                abaixo existe para o gestor sair do número agregado e entender
                quais UFs realmente puxam a priorização.
              </Typography>
            </Stack>
          </ComgepAccordionSection>
          <ComgepAccordionSection
              ...
              <Table size="small">
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell>
                      <strong>Sinais de pesquisa</strong>
                    </TableCell>
                    <TableCell align="right">
      ...
      <ComgepDetailModal
        ...
        title="Detalhamento — Lacuna e confiança do dado"
        onClose={() => setDetailModal(null)}
      >
        <Stack spacing={1.25}>
          <AiCopilotCtaRow
            ...
            {...buildComgepCopilotLinks({
              ...
              description:
                "Cobertura útil da normalização BI para cruzamento executivo por OM e UF.",
              allowAction: false,
            })}
          />
          <MeaningBlock
            title="O que isso significa"
            meaning="Este indicador mostra onde existem reportes formais sem pesquisa comparável na mesma OM e, em paralelo, mede quanto da base BI já está normalizada por OM ou UF. A ausência de pesquisa não reduz prioridade quando há reporte; apenas impede inferência de subnotificação local."
          ...
          >
            <Stack spacing={1}>
              <ModalRow
                label="Cobertura útil"
                value={`${confidencePercent.toFixed(1)}%`}
                color="#2E7D32"
              />
              <ModalRow
                label="OMs comparáveis"
                value={summary.researchComparableOmCount ?? 0}
              />
              <ModalRow
                label="OMs com reporte sem pesquisa"
                value={summary.complaintOmsWithoutResearchCount ?? 0}
                color="#4E342E"
              />
              <Typography
              ...
              >
                ...
                painel mantém o reporte formal como evidência principal e
          ...
          >
            <Stack spacing={1}>
              <ModalRow
                label="Total de registros BI"
                value={dataConfidence.totalRecords ?? 0}
              />
              <ModalRow
                label="Vínculo direto com OM"
                value={dataConfidence.matched ?? 0}
              />
              <ModalRow
                label="Vínculo apenas por UF"
                value={dataConfidence.ufOnly ?? 0}
              />
              <ModalRow
                label="Sem vínculo"
                value={dataConfidence.notFound ?? 0}
              />
            </Stack>
          </ComgepAccordionSection>
          {Array.isArray(dataConfidence.sources) &&
          dataConfidence.sources.length > 0 ? (
            <ComgepAccordionSection
            ...
            >
              <List
                disablePadding
                sx={{ border: "1px solid #E6ECF5", borderRadius: 2 }}
              >
                {dataConfidence.sources.map((item: any, index: number) => (
                  <ListItem
                    key={`${item.sourceType}-${index}`}
                    divider={index < dataConfidence.sources.length - 1}
                  >
                    <ListItemText
        ...
        <Stack spacing={1.25}>
          <AiCopilotCtaRow
            ...
            {...buildComgepCopilotLinks({
              ...
              description: selectedPriorityUf?.recommendedFocus
          ...
          <MeaningBlock
            title="O que isso significa"
            meaning="Esta UF aparece como prioritária pela combinação entre reportes formais, sinais de pesquisas normalizadas, cobertura CPCA e resposta operacional. As pesquisas só são usadas com indicação de base e ajuste conservador para reduzir viés de seleção."
            source="Fonte: cruzamento entre pesquisas BI normalizadas por OM/UF, reportes formais CPCA/SMIF e registros operacionais de missão e atividade."
          />
          <ComgepAccordionSection
            title="Resumo executivo"
          ...
          >
            <Stack spacing={1}>
              ...
              <ModalRow
                label="Faixa"
                value={selectedPriorityUf?.priorityBand ?? "—"}
                color="#ED6C02"
              />
              <ModalRow
                label="Risco"
                value={selectedPriorityUf?.riskScore ?? 0}
              />
              <ModalRow
                label="Pressão operacional"
                value={selectedPriorityUf?.pressureScore ?? 0}
                color="#D32F2F"
              />
              <ModalRow
                label="Resposta em campo"
                value={selectedPriorityUf?.presenceScore ?? 0}
              />
              <ModalRow
          ...
          >
            <Stack spacing={1}>
              <ModalRow
                label="Pesquisa escolas"
                value={`${formatPercent(selectedPriorityUf?.surveyRate)} • ${formatEvidence(selectedPriorityUf?.surveyEvidence)}`}
              />
              <ModalRow
              ...
              <ModalRow
                ...
                value={selectedPriorityUf?.sexualSignals ?? 0}
              />
              <ModalRow
                label="Assédio/violência moral"
                value={selectedPriorityUf?.moralSignals ?? 0}
              />
              <ModalRow
                label="Autor militar"
                value={selectedPriorityUf?.domesticSignals?.militaryAuthor ?? 0}
              />
              <ModalRow
                label="Subnotificação estimada"
                value={formatUnderreport(selectedPriorityUf)}
                color="#6A1B9A"
              />
              <ModalRow
                label="Reportes formais"
          ...
          selectedPriorityUf.rankingReasons.length > 0 ? (
            ...
            >
              <List
                ...
                sx={{ border: "1px solid #E6ECF5", borderRadius: 2 }}
              >
                {selectedPriorityUf.rankingReasons.map(
                  (reason: string, index: number) => (
                    <ListItem
                      key={`${reason}-${index}`}
                      divider={
                        index < selectedPriorityUf.rankingReasons.length - 1
                      }
                    >
      ...
      >
        <Stack spacing={1.25}>
          ...
          <MeaningBlock
            title="O que isso significa"
            meaning="Esta OM aparece no ranking porque há demanda formal, sinal amostral com cobertura conhecida, lacuna de cobertura CPCA ou necessidade de presença institucional. Se não houver pesquisa na OM, o ranking não presume subnotificação; mantém o reporte formal como evidência principal."
            source="Fonte: cruzamento entre pesquisas BI normalizadas por OM/UF, reportes formais CPCA/SMIF e presença operacional recente na UF."
          />
          <ComgepAccordionSection
            title="Resumo executivo"
          ...
          >
            <Stack spacing={1}>
              <ModalRow
                label="OM"
                value={selectedRiskOm?.code ?? selectedRiskOm?.name ?? "—"}
              />
              <ModalRow label="UF" value={selectedRiskOm?.uf ?? "—"} />
              <ModalRow
                label="Score de risco"
                value={selectedRiskOm?.riskScore ?? 0}
                color="#D32F2F"
              />
              <ModalRow
                label="Cobertura CPCA"
                value={formatCoverageType(selectedRiskOm?.coverageType)}
              />
              <Typography
                variant="body2"
                color="text.secondary"
              ...
              >
                {selectedRiskOm?.recommendedAction ??
                  "Monitorar cenário e manter presença institucional."}
              </Typography>
            </Stack>
          </ComgepAccordionSection>
          <ComgepAccordionSection
          ...
          >
            <Stack spacing={1}>
              <ModalRow
                label="Pesquisa escolas"
                value={`${formatPercent(selectedRiskOm?.surveyRate)} • ${formatEvidence(selectedRiskOm?.surveyEvidence)}`}
              />
              <ModalRow
                label="VD efetivo feminino 12 meses"
              ...
              <ModalRow
                label="Assédio/violência sexual"
                value={selectedRiskOm?.sexualSignals ?? 0}
              />
              <ModalRow
                label="Assédio/violência moral"
                value={selectedRiskOm?.moralSignals ?? 0}
              />
              <ModalRow
                label="Autor militar"
                value={selectedRiskOm?.domesticSignals?.militaryAuthor ?? 0}
              />
              <ModalRow
                label="Subnotificação estimada"
                value={formatUnderreport(selectedRiskOm)}
                color="#6A1B9A"
              />
              <ModalRow
                label="Reportes formais"
              ...
              <ModalRow
                label="Risco de retaliação"
                value={selectedRiskOm?.complaints?.retaliationCases ?? 0}
              />
              <ModalRow
                label="Casos além do prazo"
                value={selectedRiskOm?.complaints?.stalledCases ?? 0}
              />
              <ModalRow
                label="Missões na UF"
                value={selectedRiskOm?.operationalPresence?.missions ?? 0}
              />
              <ModalRow
                label="Atividades concluídas na UF"
                value={
                  selectedRiskOm?.operationalPresence?.completedActivities ?? 0
                }
              />
              <ModalRow
          ...
          {Array.isArray(selectedRiskOm?.rankingReasons) &&
          selectedRiskOm.rankingReasons.length > 0 ? (
            <ComgepAccordionSection
            ...
            >
              <List
                ...
                sx={{ border: "1px solid #E6ECF5", borderRadius: 2 }}
              >
                {selectedRiskOm.rankingReasons.map(
                  (reason: string, index: number) => (
                    <ListItem
                      key={`${reason}-${index}`}
                      divider={index < selectedRiskOm.rankingReasons.length - 1}
                    >
                      <ListItemText primary={reason} />
                    </ListItem>
                  ),
