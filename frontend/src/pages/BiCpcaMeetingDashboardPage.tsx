...
type CpcaMeetingDashboardResponse = {
  kpis: {
...
const CHART_COLORS = [
  CPCA_BI_PALETTE.primary,
  CPCA_BI_PALETTE.secondary,
  CPCA_BI_PALETTE.accent,
  "#4B6CB7",
  "#7B8E3E",
  "#9C6644",
  "#5B8FB9",
...
const cardSx = {
  borderRadius: 3,
  border: `1px solid ${alpha(CPCA_BI_PALETTE.primary, 0.12)}`,
  boxShadow: `0 12px 28px ${alpha(CPCA_BI_PALETTE.primary, 0.08)}`,
...
const axisTickStyle = {
  fill: CPCA_BI_PALETTE.muted,
  fontSize: 12,
};

const chartGridStroke = alpha(CPCA_BI_PALETTE.primary, 0.14);
const chartAxisStroke = alpha(CPCA_BI_PALETTE.primary, 0.2);
const tooltipContentStyle = {
  borderRadius: 10,
  border: `1px solid ${alpha(CPCA_BI_PALETTE.primary, 0.2)}`,
  boxShadow: `0 10px 24px ${alpha(CPCA_BI_PALETTE.primary, 0.15)}`,
  background: "#FFFFFF",
};
const tooltipLabelStyle = { color: CPCA_BI_PALETTE.text, fontWeight: 700 };
...
export function BiCpcaMeetingDashboardPage() {
  ...
  const dashboardQuery = useBiCpcaMeetingDashboard(dashboardFilters);
  const responsesQuery = useBiCpcaMeetingResponses({
    ...dashboardFilters,
    page,
    pageSize: 25,
  });
  ...
  const exportCurrentCsv = () => {
    ...
    a.href = url;
    a.download = "bi-encontro-cpca-recorte.csv";
    ...
    >
      ...
      >
        <Box>
          ...
          <Typography variant="body2" sx={{ color: CPCA_BI_PALETTE.muted }}>
                      ...
                      {list.items.map((item) => (
                        <Box
                          ...
                          sx={{
                            borderRadius: 2,
                            p: 1,
                            border: `1px solid ${alpha(CPCA_BI_PALETTE.primary, 0.18)}`,
                            background: alpha(CPCA_BI_PALETTE.primary, 0.03),
                          }}
                        >
